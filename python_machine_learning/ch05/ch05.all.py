...
[MIT License](https://github.com/rasbt/python-machine-learning-book/blob/master/LICENSE.txt)# Python Machine Learning - Code Examples# Chapter 5 - Compressing Data via Dimensionality ReductionNote that the optional watermark extension is a small IPython notebook plugin that I developed to make the code reproducible. You can just skip the following line(s).%load_ext watermark
%watermark -a 'Sebastian Raschka' -u -d -v -p numpy,scipy,matplotlib,sklearn*The use of `watermark` is optional. You can install this IPython extension via "`pip install watermark`". For more information, please see: https://github.com/rasbt/watermark.*<br>
<br>### Overview- [Unsupervised dimensionality reduction via principal component analysis 128](#Unsupervised-dimensionality-reduction-via-principal-component-analysis-128)
...
from sklearn import __version__ as sklearn_version# Unsupervised dimensionality reduction via principal component analysisImage(filename='./images/05_01.png', width=400) import pandas as pd
...
df_wine.head()<hr>Splitting the data into 70% training and 30% test subsets.if Version(sklearn_version) < '0.18':
...
<br>
<br>## Total and explained variancetot = sum(eigen_vals)
...
plt.show()<br>
<br>## Feature transformation# Make a list of (eigenvalue, eigenvector) tuples
...
# first element of the tuples, now).w = np.hstack((eigen_pairs[0][1][:, np.newaxis],
...
plt.show()X_train_std[0].dot(w)<br>
<br>## Principal component analysis in scikit-learnfrom sklearn.decomposition import PCA
...
plt.ylabel('Explained variance ratio')
plt.xlabel('Principal components')
plt.show()pca = PCA(n_components=2)
X_train_pca = pca.fit_transform(X_train_std)
...
def plot_decision_regions(X, y, classifier, resolution=0.02):
    ...
    for idx, cl in enumerate(np.unique(y)):
        plt.scatter(x=X[y == cl, 0], y=X[y == cl, 1],
                    ...
                    marker=markers[idx], label=cl)Training logistic regression classifier using the first 2 principal components.from sklearn.linear_model import LogisticRegression
...
plt.show()plot_decision_regions(X_test_pca, y_test, classifier=lr)
plt.xlabel('PC 1')
plt.ylabel('PC 2')
plt.legend(loc='lower left')
plt.tight_layout()
# plt.savefig('./figures/pca4.png', dpi=300)
plt.show()pca = PCA(n_components=None)
X_train_pca = pca.fit_transform(X_train_std)
pca.explained_variance_ratio_<br>
<br># Supervised data compression via linear discriminant analysisImage(filename='./images/05_06.png', width=400) <br>
<br>## Computing the scatter matricesCalculate the mean vectors for each class:np.set_printoptions(precision=4)
...
print('Within-class scatter matrix: %sx%s' % (S_W.shape[0], S_W.shape[1]))Better: covariance matrix since classes are not equally distributed:print('Class label distribution: %s' 
      % np.bincount(y_train)[1:])d = 13  # number of features
S_W = np.zeros((d, d))
for label, mv in zip(range(1, 4), mean_vecs):
...
<br>## Selecting linear discriminants for the new feature subspaceSolve the generalized eigenvalue problem for the matrix $S_W^{-1}S_B$:eigen_vals, eigen_vecs = np.linalg.eig(np.linalg.inv(S_W).dot(S_B))**Note**:
...
for eigen_val in eigen_pairs:
    print(eigen_val[0])tot = sum(eigen_vals.real)
...
plt.show()w = np.hstack((eigen_pairs[0][1][:, np.newaxis].real,
              eigen_pairs[1][1][:, np.newaxis].real))
print('Matrix W:\n', w)<br>
<br>## Projecting samples onto the new feature spaceX_train_lda = X_train_std.dot(w)
colors = ['r', 'b', 'g']
markers = ['s', 'x', 'o']
...
plt.show()<br>
<br>## LDA via scikit-learnif Version(sklearn_version) < '0.18':
    from sklearn.lda import LDA
else:
    from sklearn.discriminant_analysis import LinearDiscriminantAnalysis as LDA

lda = LDA(n_components=2)
X_train_lda = lda.fit_transform(X_train_std, y_train)from sklearn.linear_model import LogisticRegression
lr = LogisticRegression()
...
plt.show()<br>
<br># Using kernel principal component analysis for nonlinear mappingsImage(filename='./images/05_11.png', width=500) <br>
<br>## Implementing a kernel principal component analysis in Pythonfrom scipy.spatial.distance import pdist, squareform
...
def rbf_kernel_pca(X, gamma, n_components):
    ...
    return X_pc<br>### Example 1: Separating half-moon shapesimport matplotlib.pyplot as plt
...
plt.show()from matplotlib.ticker import FormatStrFormatter
...
plt.show()<br>### Example 2: Separating concentric circlesfrom sklearn.datasets import make_circles
...
plt.show()scikit_pca = PCA(n_components=2)
X_spca = scikit_pca.fit_transform(X)
...
plt.show()X_kpca = rbf_kernel_pca(X, gamma=15, n_components=2)
...
plt.show()<br>
<br>## Projecting new data pointsfrom scipy.spatial.distance import pdist, squareform
from scipy import exp
from scipy.linalg import eigh
import numpy as np
...
def rbf_kernel_pca(X, gamma, n_components):
    ...
    return alphas, lambdasX, y = make_moons(n_samples=100, random_state=123)
alphas, lambdas = rbf_kernel_pca(X, gamma=15, n_components=1)x_new = X[-1]
x_newx_proj = alphas[-1] # original projection
x_projdef project_x(x_new, X, gamma, alphas, lambdas):
...
x_reproj plt.scatter(alphas[y == 0, 0], np.zeros((50)),
            color='red', marker='^', alpha=0.5)
...
plt.show()X, y = make_moons(n_samples=100, random_state=123)
...
# plt.savefig('./figures/reproject.png', dpi=300)
plt.show()<br>
<br>## Kernel principal component analysis in scikit-learnfrom sklearn.decomposition import KernelPCA
